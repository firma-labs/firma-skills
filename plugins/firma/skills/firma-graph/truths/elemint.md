---
name: elemint
description: An elemint is one non-transferable fingerprint of real production, e(photon) e(H2O) e(Au) e(BTC), signed to its host's Embodimint. Never a token you hold. A vault claim is the tradeable piece, one per vaulted elemint.
type: truth
status: LOCKED 2026-09-05
source: firma-blueprint rulings D2, D11; firma-vault wiki/elemint-master-knowledge.md
---

# Elemint

**An elemint** is a digital fingerprint of real production, matched to whoever or wherever produced it — a person, an agent (Ambassadors included), or a place. It is signed to the host's [[embodimint]], never moves, and is never a token you hold or trade. "Digital fingerprints matching you, not tokens you hold." Notation: **e(photon)**, **e(compute)**, **e(H2O)**, **e(Au)**, **e(terra)**, **e(BTC)**. The parentheses are part of the name. Never `$eWATT`, never `eWATT` in body copy.

**A vault claim** (word PROPOSED; Curtis may pick a Firma word) is the tradeable piece, issued only against a **vaulted** elemint, one claim per elemint (single-encumbrance registry). Claims trade on e(xchange) in FIG and count toward FIG backing. Three rings: fingerprint / vault claim / market.

**Elemint (capital)** is the family: treasury and custody, the RWA chain with the custody state machine (in-ground → raw → transit → refined → vaulted), RWA banking, trading, exploration and mining, the RWA fund. This is its 2023 shape and the Blueprint draws it that way.

**Exploration** is not a metaphor. It is a real capability already in hand: a Digital Signature survey that reads the earth's electro-magnetic signature and returns where a resource sits in place — drill coordinates, depth, quality, flow rate — without drilling. It found water on royal land in Saudi Arabia in 2023 and every called spot was there. The archive is the Drive folder "Elemint Exploration": the Global Water Solutions / Inteletec brief ATB-220809, the corporate profile, and delivered target packages for DRC Kinshasa (2022) and Nepal (2019). The survey is what reads a parcel before Elemint puts its title on chain, and it feeds the e(terra) and e(H2O) fingerprints. Blueprint part L1-25h.

**Land title is Elemint's too, from 2026-09-19.** The [[liquid-terra]] name is retired and its work sits here under the Elemint name: on-chain title for a parcel, an SPV around it, and an **e(terra)** attestation for each. The capital-formation chain reads **NS Simulator → Investmint → Elemint → Settlemint → [[firmamint]]**, no debt anywhere in it. Blueprint part L4-42 still carries the old title until `sites/blueprint/data` is renamed.

**The family, ruled 2026-09-19.** Three names, no fourth. **Elemint** is the platform: what a real asset is and who holds it. **Elemint Exploration** runs the surveys — the Digital Signature capability above — and holds the survey technology agreement. **Elemint Holdings** holds the reserve slice on behalf of the Firma Fund. Curtis: "adding another name in like liquid terra doesn't make sense — our RWA platform needs to just be Elemint." A partner's own marketplace keeps its own name and runs on Elemint; no third brand sits between them. Curtis ruled the same day that [[liquid-terra]] retires as a name and its land-title work folds in here.

Elemints back [[fig]]. Hosts do not sell production; they give excess, and the fingerprint is minted in their name. See [[earning-by-giving]] and [[atonemint]].
